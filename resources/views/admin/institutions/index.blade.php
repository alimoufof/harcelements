    ...
    <!--  Header BreadCrumb -->
     <div class="content-header">
         ...
         <div class="create-item">
             <a href="{{ route('institution.create') }}" class="theme-primary-btn btn btn-primary"><i class="material-icons">add</i>Ajouter une institution</a>
                        ...
                        <table id="usersTable" class="table table-striped table-borderless" style="width:100%">
                            ...
                            <tbody>
                                @foreach($institutions as $key => $institution)
                                    <tr>
                                        <td>{{ $key + 1 }}</td>
                                        <td>{{ $institution->user->nom . ' ' . $institution->user->prenom }}</td>
                                        <td>{{ $institution->type }}</td>
                                        <td>{{ $institution->nom }}</td>
                                        <td>
                                            <div class="d-flex gap-2 justify-content-end">
                                                <a class="btn btn-sm btn-secondary" href="{{route('institution.show', $institution)}}">Voir</a>  
                                                <a class="btn btn-sm btn-info" href="{{route('institution.edit', $institution)}}">Modifier</a>
